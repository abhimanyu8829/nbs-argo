...
- Split platform infrastructure into separate Helm charts under `helm/` for MetalLB config, Postgres, PgBouncer, Redis, Traefik, and OPA Gatekeeper policies.
...
- Updated `argocd/root-app.yaml` so ArgoCD can sync the app-of-apps chart from Git.
- Added `argocd/apps/chart-tags.yaml` as the single file automation should update for ArgoCD platform chart versions.
...
- Updated `script/installation/setup-vm.sh` to install missing prerequisites conditionally and bootstrap the platform through Helm plus ArgoCD.
- Added `script/push-infra-charts.sh` to package and push all infra charts to ECR.
