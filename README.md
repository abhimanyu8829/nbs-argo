...
8. **ArgoCD automatically detects the new chart version** (because `Legacy yaml/argocd-app.yaml` points at `oci://…` with `automated` sync) and rolls out a **RollingUpdate** of all services.
9. **Verify**:
   ```bash
   kubectl get pods -A
...
- **`Legacy yaml/argocd-app.yaml`** now uses `repoURL: oci://798701233691.dkr.ecr.ap-south-1.amazonaws.com`.
...
| `Legacy yaml/argocd-app.yaml` | `targetRevision` | Must match the `version` in `Chart.yaml` (e.g., `1.0.3`) |
...
| Rollback does not happen | Confirm the older chart version exists in the ECR OCI registry and that `Legacy yaml/argocd-app.yaml` points at the correct `targetRevision`. |
